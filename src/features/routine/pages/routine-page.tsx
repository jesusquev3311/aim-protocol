...
export function RoutinePage() {
  ...
  const targetAchieved = isRoutineComplete(values);
  ...
  return <section className="space-y-8"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-semibold uppercase tracking-widest text-primary">Routine tracker</p><h1 className="mt-2 text-3xl font-bold">30-minute mechanics protocol</h1><p className="mt-2 max-w-3xl text-muted-foreground">Training—not a warm-up. Combine mouse aim, movement, a complete stop, and a deliberate shot.</p></div><div className="space-y-2"><Label htmlFor="routine-date">Session date</Label><Input id="routine-date" type="date" max={today} value={selectedDate} onChange={(event) => setSelectedDate(event.target.value)} /></div></div>
    <div className="grid gap-4 sm:grid-cols-3"><Metric icon={<Timer />} label="Daily routine" value={`${progress}%`} detail={dayFinished ? `Finished${targetAchieved ? " · all targets achieved" : " · partial session"}` : "Session still open"} /><Metric icon={<Trophy />} label="Ranked this week" value={`${weeklyRanked} / 14`} detail={`${formatShortDate(week.start)} – ${formatShortDate(week.end)}`} /><Metric icon={<Flame />} label="Routine streak" value={`${streak} ${streak === 1 ? "day" : "days"}`} detail="Finished consecutive sessions" /></div>
    <Card className={dayFinished ? "border-emerald-500/40" : "border-primary/30"}><CardContent className="pt-6"><div className="mb-2 flex justify-between text-sm"><span>{dayFinished ? `Day finished · ${targetAchieved ? "all targets achieved" : "partial result recorded"}` : "Daily protocol progress"}</span><span>{progress}%</span></div><div className="h-3 overflow-hidden rounded-full bg-muted"><div className={`h-full rounded-full transition-all ${dayFinished ? "bg-emerald-500" : "bg-primary"}`} style={{ width: `${progress}%` }} /></div>{dayFinished && session.data?.completed_at && <p className="mt-3 text-xs text-muted-foreground">Finished {new Intl.DateTimeFormat("en", { dateStyle: "medium", timeStyle: "short" }).format(new Date(session.data.completed_at))}</p>}</CardContent></Card>
